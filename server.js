...
app.get('/start-scene', cors(), async (req, res) => {
  ...
  const timeRemaining = 5 * 60 * 1000 - timeSinceLastExecution;
  ...
  try {
    ...
    // Externe URL auch aufrufen
    await controlDevice('https://run.chayns.codes/90ce9d54'); // Die zusätzliche URL wird jetzt aufgerufen
